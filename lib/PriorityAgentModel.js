...
var probTypeA = 0.05; // updated from 0.5
...
(function() {
	...
	document.getElementById("title").textContent = "Ku Wee Tiong 1002713"
	redrawWindow();
})();
...
// these next 2 functions are for Priority logic
function checkPriority(patient){
	// checks a row in patient table for specified priority conditions
	return (patient.type == "A" &&
			patient.id == nextTreatedPatientID_A &&
			patient.state == STAGING &&
			patient.location.row == doctorRow-1 &&
			patient.location.col == doctorCol-1)
}

function thereIsPriorityPatients(){
	// to check if at least one element in the array meets the condition
	return patients.some(checkPriority); // true if at least one patient fits conditions
}
	...
	switch(state){
		...
		case STAGING:
			...
			if (hasArrived){
				...
				// priority logic: to produce isnext = true or false
				if (patient.type=="A"){
					var isnext = true;
				}
				else { // patient.type=="B"
					...
					var isnext = false
					// but true if no priority patient presently
					if (!thereIsPriorityPatients()) {isnext = true}
					}
				
				// move STAGING -> INTREATMENT if ixnext==true
				if (isnext){
					...
					if (doctor.state == IDLE){
						...
						doctor.state = BUSY; // update doctor & patient states to busy
						patient.state = INTREATMENT;
						patient.target.row = doctorRow; // set positions for patient
						patient.target.col = doctorCol;
						if (patient.type == "A") nextTreatedPatientID_A++; else nextTreatedPatientID_B++; // update counter
					}
				}
			}
		break;
